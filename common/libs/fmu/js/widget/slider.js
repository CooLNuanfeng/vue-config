...
/**
 * @file slider
 * @import vendors/jquery/jquery.js,core/fmu.js, core/widget.js
 */
...
FMU.UI.Slider = function(container, options) {
    ...
    options.imgLazyLoad = options.imgLazyLoad !== undefined ? options.imgLazyLoad : true;

    options.showDot = options.showDot !== undefined ? options.showDot : true;
    if (options.showDot) {
        var dotStringBuilder = new StringBuilder();
        var dotOnClass = "fmu-dot-on";

        dotStringBuilder.append('<section class="fmu-slider-dots-box J_fmu_slider_dots_box">');
        ...
        $dot = $container.next(".J_fmu_slider_dots_box").find(".J_fmu_dot");
        $dot.on("touchend", function () {
            var $this = $(this);
            if ($this.hasClass(dotOnClass)) {
                return;
            }
            $dot.removeClass(dotOnClass);
    ...
    function imgLazyLoad(ele) {
        var $img = $(ele).find("img[data-original]");
        $img.length > 0 ? $img.attr("src", $img.data("original")).removeAttr("data-original") : '';
    }
    ...
    function slide(to, slideSpeed) {
        ...
        if (browser.transitions && options.continuous && length < 3) {
            options.showDot && $dot.removeClass(dotOnClass).eq(index % 2).addClass(dotOnClass);
        } else {
            options.showDot && $dot.removeClass(dotOnClass).eq(index).addClass(dotOnClass);
        }
    ...
    function move(index, dist, speed) {
        ...
        slidePos[index] = dist;
        (dist === 0 && options.imgLazyLoad) ? imgLazyLoad(slides[index]) : '';
    }
