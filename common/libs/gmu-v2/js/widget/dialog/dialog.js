    ...
    gmu.define( 'Dialog', {
        ...
        _calculate: function(){
            var me = this, opts = me._options, size, $win, root = document.body,
                ret = {}, isBody = opts._cIsBody, round = Math.round, margintop;
			...
			//当window高度小于dialog高度时，强制可以scroll，且marginTop为-win.height/2
			if( $win.height()<size.height ){
				opts.scrollMove = false; 
				margintop = -round($win.height()/2) +'px';
			}else{
        ...
        open: function(x, y){
            ...
            opts._wrap.css("min-width", "0px").css('display', 'block');
